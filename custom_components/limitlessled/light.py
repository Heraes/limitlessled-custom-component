...
from homeassistant.const import CONF_HOST, CONF_NAME, CONF_PORT, CONF_TYPE, STATE_ON, CONF_UNIQUE_ID
    ...
    for bridge_conf in config[CONF_BRIDGES]:
        ...
        for group_conf in bridge_conf[CONF_GROUPS]:
            ...
            lights.append(LimitlessLEDGroup(group, {
                ...
                "unique_id": group_conf.get(CONF_UNIQUE_ID),
            }))
    add_entities(lights)
...
class LimitlessLEDGroup(LightEntity, RestoreEntity):
    ...
    def __init__(self, group: Group, config: dict[str, Any]) -> None:
        ...
        # Set unique ID from config or generate fallback
        self._attr_unique_id = config.get("unique_id") or f"limitlessled_{group.name}_{group.number}"
